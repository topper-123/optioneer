...
class TestConfig(object):
    ...
    def test_api(self):
        # the config object exposes the user API
        assert hasattr(self.cf, 'get_option')
        assert hasattr(self.cf, 'set_option')
        assert hasattr(self.cf, 'reset_option')
    ...
    def test_describe_option(self):
        ...
        self.cf.deprecate_option('g.h', redirect_key="k")
        ...
        assert 'doc' in self.cf.describe_option('a', print_desc=False)
        assert 'doc2' in self.cf.describe_option('b', print_desc=False)
        assert 'precated' in self.cf.describe_option('b', print_desc=False)
        assert 'doc3' in self.cf.describe_option('c.d.e1', print_desc=False)
        assert 'doc4' in self.cf.describe_option('c.d.e2', print_desc=False)

        # if no doc is specified we get a default message
        # saying "description not available"
        assert 'vailable' in self.cf.describe_option('f', print_desc=False)
        assert 'vailable' in self.cf.describe_option('g.h', print_desc=False)
        assert 'precated' in self.cf.describe_option('g.h', print_desc=False)
        assert 'k' in self.cf.describe_option('g.h', print_desc=False)

        # default is reported
        assert 'foo' in self.cf.describe_option('l', print_desc=False)
        # current value is reported
        assert 'bar' not in self.cf.describe_option('l', print_desc=False)
        self.cf.set_option("l", "bar")
        assert 'bar' in self.cf.describe_option('l', print_desc=False)

    def test_case_insensitive(self):
        self.cf.register_option('KanBAN', 1, 'doc')

        assert 'doc' in self.cf.describe_option('kanbaN', print_desc=False)
    ...
    def test_deprecate_option(self):
        ...
        self.cf.deprecate_option('d.dep', redirect_key='d.a')  # reroute d.dep to d.a
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter('always')
    ...
    def test_config_prefix(self):
        ...
        assert 'doc1' in self.cf.describe_option('base.a', print_desc=False)
        assert 'doc2' in self.cf.describe_option('base.b', print_desc=False)
    ...
    def test_callback(self):
        ...
        self.cf.register_option('d.a', 'foo', callback=callback)
        self.cf.register_option('d.b', 'foo', callback=callback)
    ...
    def test_attribute_access(self):
        ...
        self.cf.register_option('a', 0)
        self.cf.register_option('c', 0, callback=f3)
