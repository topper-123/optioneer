...
class DictWrapper(object):
    ...
    def __setattr__(self, key, val):
        ...
        if key in self.d and not isinstance(self.d[key], dict):
            self._config._set_option(prefix, val)
        else:
            raise OptioneerError("You can only set the value of existing options")
    ...
    def __getattr__(self, key):
        ...
        if isinstance(v, dict):
            return self.__class__(self._config, v, prefix)
        else:
    ...
    def inner(x):
        ...
        if x not in legal_values:
            ...
            if not any(c(x) for c in callables):
                pp_values = pp("|".join(map(pp, legal_values)))
...
class Optioneer:
    ...
    def _describe_option(self, pat='', _print_desc=True):
        ...
        for k in keys:  # filter by pat
            s += self._build_option_description(k)
    ...
    def _reset_option(self, pat, silent=False):
        ...
        for k in keys:
            self._set_option(k, self._registered_options[k].defval, silent=silent)

    def get_default_val(self, pat):
        key = self._get_single_key(pat, silent=True)
        return self._get_registered_option(key).defval
    ...
    def register_option(self, key, defval, doc='', validator=None, cb=None):
        ...
        validator - a function of a single argument, should raise `ValueError`
                    if called with a value which is not a legal value for the
                    option.
        ...
        Raises
        ------
        ValueError if `validator` is set and `defval` is not a valid value.
    ...
    def deprecate_option(self, key, msg=None, rkey=None, removal_ver=None):
        """
        ...
        key - the name of the option to be deprecated. must be a
                fully-qualified option name (e.g "x.y.z.rkey").
    ...
    def _get_deprecated_option(self, key):
        ...
        try:
            d = self._deprecated_options[key]
        except KeyError:
            return None
        else:
            return d
    ...
    def _translate_key(self, key):
        ...
        d = self._get_deprecated_option(key)
        if d:
    ...
    def _build_option_description(self, k):
        """
        Builds a formatted description of a registered option and prints it.
        """

        o = self._get_registered_option(k)
        d = self._get_deprecated_option(k)
        ...
        if o:
            s += (u'\n    [default: {default}] [currently: {current}]'
                  .format(default=o.defval, current=self._get_option(k, True)))

        if d:
            s += u'\n    (Deprecated'
            s += (u', use `{rkey}` instead.'
                  .format(rkey=d.rkey if d.rkey else u''))
            s += u')'
